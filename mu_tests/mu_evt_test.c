...
void mu_task_test() {
  ...
  UTEST_ASSERT(mu_task_is_immediate(&event_i) == true);
  UTEST_ASSERT(mu_task_is_immediate(&event_t1) == false);
  UTEST_ASSERT(mu_task_is_immediate(&event_t2) == false);

  UTEST_ASSERT(mu_task_time(&event_t1) == now);
  UTEST_ASSERT(mu_task_time(&event_t2) == then);

  UTEST_ASSERT(mu_task_is_runnable(&event_i, now) == true);
  UTEST_ASSERT(mu_task_is_runnable(&event_t1, now) == true);
  UTEST_ASSERT(mu_task_is_runnable(&event_t2, now) == false);

  // Events can always be run even if time has not yet arrived
  mu_task_call(&event_i, (void *)1);
  mu_task_call(&event_t1, (void *)2);
  mu_task_call(&event_t2, (void *)3);

  UTEST_ASSERT(s_msg_i_called == 1);
  UTEST_ASSERT(s_msg_t1_called == 1);
  UTEST_ASSERT(s_msg_t2_called == 1);

  UTEST_ASSERTEQ_BOOL(mu_task_is_after(&event_i, &event_t1), false);
  UTEST_ASSERTEQ_BOOL(mu_task_is_after(&event_t1, &event_i), true);

  UTEST_ASSERTEQ_BOOL(mu_task_is_after(&event_t1, &event_t2), false);
  UTEST_ASSERTEQ_BOOL(mu_task_is_after(&event_t2, &event_t1), true);

  UTEST_ASSERTEQ_BOOL(mu_task_is_after(&event_i, &event_i), false);
  UTEST_ASSERTEQ_BOOL(mu_task_is_after(&event_t1, &event_t1), false);
}
